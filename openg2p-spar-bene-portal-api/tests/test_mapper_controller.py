...
async def test_link_endpoint():
    ...
    with patch.object(
        controller.service, "link", new_callable=AsyncMock
    ) as mock_service_link, patch(
        ...
        mock_service_link.return_value = [
...
async def test_resolve_endpoint():
    ...
    # Mock the service and helper methods
    with patch.object(
        controller.service, "resolve", new_callable=AsyncMock
    ) as mock_service_resolve, patch(
        "openg2p_spar_bene_portal_api.controllers.mapper_controller.RequestValidation"
    ) as mock_validation, patch(
        "openg2p_spar_bene_portal_api.controllers.mapper_controller.StrategyHelper"
    ) as mock_strategy_helper, patch(
        ...
        mock_service_resolve.return_value = [
...
async def test_update_endpoint():
    ...
    mock_request.request_body.request_payload.update_request = [
        mock_single_update_request
    ]

    # Mock the service and helper methods
    with patch.object(
        controller.service, "update", new_callable=AsyncMock
    ) as mock_service_update, patch(
        "openg2p_spar_bene_portal_api.controllers.mapper_controller.RequestValidation"
    ) as mock_validation, patch(
        "openg2p_spar_bene_portal_api.controllers.mapper_controller.StrategyHelper"
    ) as mock_strategy_helper, patch(
        ...
        mock_strategy_helper_instance = MagicMock()
        mock_strategy_helper_instance.construct_id = AsyncMock(
            return_value="constructed_id_789"
        )
        mock_strategy_helper.return_value.get_component.return_value = (
            mock_strategy_helper_instance
        )
        ...
        mock_response = MagicMock()
        mock_response_helper.get_component.return_value.construct_update_response.return_value = (
            mock_response
        )
        mock_service_update.return_value = [
            MagicMock(reference_id="ref123", status=StatusEnum.succ)
        ]
...
async def test_unlink_endpoint():
    ...
    mock_request.request_body.request_payload.unlink_request = [
        mock_single_unlink_request
    ]

    # Mock the service and helper methods
    with patch.object(
        controller.service, "unlink", new_callable=AsyncMock
    ) as mock_service_unlink, patch(
        "openg2p_spar_bene_portal_api.controllers.mapper_controller.RequestValidation"
    ) as mock_validation, patch(
        "openg2p_spar_bene_portal_api.controllers.mapper_controller.StrategyHelper"
    ) as mock_strategy_helper, patch(
        ...
        mock_strategy_helper_instance = MagicMock()
        mock_strategy_helper_instance.construct_id = AsyncMock(
            return_value="constructed_id_101"
        )
        mock_strategy_helper.return_value.get_component.return_value = (
            mock_strategy_helper_instance
        )
        ...
        mock_response = MagicMock()
        mock_response_helper.get_component.return_value.construct_unlink_response.return_value = (
            mock_response
        )
        mock_service_unlink.return_value = [
            MagicMock(reference_id="ref123", status=StatusEnum.succ)
        ]
...
async def test_link_error_handling():
    ...
    with patch(
        ...
        mock_response_helper.get_component.return_value.construct_error_response.return_value = (
            MagicMock()
        )
