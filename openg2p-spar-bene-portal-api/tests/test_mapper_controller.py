...
async def test_link_endpoint():
    ...
    with patch("openg2p_spar_bene_portal_api.controllers.mapper_controller.MapperService") as mock_mapper_service, \
        ...
        mock_service_instance.link = AsyncMock(return_value=[MagicMock(reference_id="ref123", status=StatusEnum.succ)])
...
async def test_resolve_endpoint():
    ...
    with patch("openg2p_spar_bene_portal_api.controllers.mapper_controller.MapperService") as mock_mapper_service, \
        ...
        # Setup service mock
        mock_service_instance = MagicMock()
        mock_service_instance.resolve = AsyncMock(return_value=[MagicMock(reference_id="ref123", fa="FA456", status=StatusEnum.succ)])
        mock_mapper_service.get_component.return_value = mock_service_instance
